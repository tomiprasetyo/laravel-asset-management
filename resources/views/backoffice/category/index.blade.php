...
@section('content')
    <x-container>
        <div class="col-12 col-lg-8">
            <x-card-action title="Daftar Kategori"
                url="{{ route('backoffice.category.index') }}>
        <x-table>
          <thead>
            <tr>
              <th>#</th>
...
<tr>
                ...
                <td>
                <span class="avatar
                rounded avatar-md" style="background-image: url({{ $category->image }})"></span>
                </td>
                <td>{{ $category->name }}</td>
                <td>
                    @can('category-update')
                        ...
                        <x-modal id="{{ $category->id }}" title="Ubah Data">
                            <form action="{{ route('backoffice.category.update', $category->id) }}" method="POST"
